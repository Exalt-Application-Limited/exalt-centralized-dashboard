...
# Development utility script for centralized-core
...
start_service() {
    echo "Starting $centralized-core..."
    cd "$SERVICE_DIR"
    ...
    nohup java -jar target/${centralized-core}-*.jar > logs/app.log 2>&1 &
    echo $! > .pid
    
    echo "$centralized-core started with PID $(cat .pid)"
    echo "Logs available at: logs/app.log"
}

# Function to stop the service
stop_service() {
    echo "Stopping $centralized-core..."
    
    if [ -f .pid ]; then
        PID=$(cat .pid)
        if ps -p $PID > /dev/null; then
            kill $PID
            echo "$centralized-core stopped"
        else
            echo "$centralized-core is not running"
        fi
        rm -f .pid
    else
        echo "PID file not found. $centralized-core may not be running"
    fi
...
dev_mode() {
    echo "Starting $centralized-core in development mode..."
    cd "$SERVICE_DIR"
    docker-compose up -d
...
health_check() {
    echo "Checking $centralized-core health..."
