...
# Development utility script for centralized-dashboard-shared
...
start_service() {
    echo "Starting $centralized-dashboard-shared..."
    cd "$SERVICE_DIR"
    ...
    nohup java -jar target/${centralized-dashboard-shared}-*.jar > logs/app.log 2>&1 &
    echo $! > .pid
    
    echo "$centralized-dashboard-shared started with PID $(cat .pid)"
    echo "Logs available at: logs/app.log"
}

# Function to stop the service
stop_service() {
    echo "Stopping $centralized-dashboard-shared..."
    
    if [ -f .pid ]; then
        PID=$(cat .pid)
        if ps -p $PID > /dev/null; then
            kill $PID
            echo "$centralized-dashboard-shared stopped"
        else
            echo "$centralized-dashboard-shared is not running"
        fi
        rm -f .pid
    else
        echo "PID file not found. $centralized-dashboard-shared may not be running"
    fi
...
run_tests() {
    echo "Running tests for $centralized-dashboard-shared..."
    cd "$SERVICE_DIR"
...
dev_mode() {
    echo "Starting $centralized-dashboard-shared in development mode..."
    cd "$SERVICE_DIR"
    docker-compose up -d
...
health_check() {
    echo "Checking $centralized-dashboard-shared health..."
...
service_info() {
    ...
    echo "==================="
    echo "Name: $centralized-dashboard-shared"
